...
class HardwareSource:
    ...
    def collect(
        ...
        *,
        ...
        feedback_stale_timeout_s = max(
            nominal_feedback_cycle_s * float(self.config.serial.feedback_stale_timeout_factor),
            0.1,
        )
        ...
        log_info(
            "串口回传优先保留覆盖窗口诊断；控制推进按最新完整快照进行，"
            "只要 active_joints 全部初始化且反馈未陈旧，就不会被固定相位错位卡住。"
        )
    ...
    def prepare_identification(self, data: CollectedData) -> IdentificationInputs | None:
        ...
        clean_mask = np.any(joint_clean_mask[:, self.config.active_joint_mask], axis=1)
        retained_per_joint = np.count_nonzero(joint_clean_mask, axis=0)
        active_retained = retained_per_joint[self.config.active_joint_mask]
        ...
        retained_summary = ", ".join(
            ...
            for joint_idx in self.config.active_joint_indices
        )
        ...
        qd_fit = qd_filtered[:, self.config.active_joint_mask]
        tau_fit = tau_residual[:, self.config.active_joint_mask]
        sample_mask = joint_clean_mask[:, self.config.active_joint_mask]
