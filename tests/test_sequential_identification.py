...
class SequentialIdentificationTests(unittest.TestCase):
    def test_parser_and_command_adapter(self) -> None:
        ...
        adapter = SingleMotorCommandAdapter(motor_count=7)
        command = adapter.pack(3, 1.25)
        payload_values = COMMAND_PAYLOAD_STRUCT.unpack(command[2:30])
        self.assertAlmostEqual(payload_values[2], 1.25, places=6)
        self.assertTrue(all(abs(value) < 1.0e-9 for index, value in enumerate(payload_values) if index != 2))
    ...
    def test_reference_trajectory_covers_expected_phases(self) -> None:
        ...
        excitation = replace(
            ...
            hold_end=0.2,
            amplitude=0.4,
            frequency_bands=(0.5, 1.0),
        )
        trajectory = build_reference_trajectory(excitation)
        self.assertEqual(str(trajectory.phase_name[0]), "hold_start")
        self.assertIn("forward_band_01", set(trajectory.phase_name.tolist()))
        self.assertIn("reverse_band_01", set(trajectory.phase_name.tolist()))
        self.assertEqual(str(trajectory.phase_name[-1]), "hold_end")
        self.assertLessEqual(float(np.max(np.abs(trajectory.position_cmd))), 0.4 + 1.0e-6)
        self.assertAlmostEqual(float(trajectory.position_cmd[0]), 0.0, places=6)
        self.assertAlmostEqual(float(trajectory.position_cmd[-1]), 0.0, places=6)
