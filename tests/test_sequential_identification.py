...
from friction_identification_core.__main__ import build_parser
...
""",
    ...
    def test_readme_and_cli_match_sequential_mode(self) -> None:
        parser = build_parser()
        mode_action = next(action for action in parser._actions if action.dest == "mode")
        self.assertEqual(tuple(mode_action.choices), ("default", "sequential"))

        readme = README_PATH.read_text(encoding="utf-8")
        self.assertIn("CLI 入口只有 `default` 和 `sequential`", readme)
        self.assertIn("`default` 和 `sequential` 是同一条主流程", readme)
    ...
    def test_capture_waits_for_sync_before_sampling(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertGreater(int(metadata["sequence_error_count"]), 0)
    ...
    def test_end_to_end_pipeline_writes_quality_metrics_and_summary(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(manifest["mode"], "sequential")
    ...
    def test_rerun_recorder_logs_quality_and_conclusion_paths(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with mock.patch("friction_identification_core.visualization.rr", fake_rr):
                recorder = RerunRecorder(summary_path.with_suffix(".rrd"), motor_ids=(1,), motor_names={1: capture.motor_name})
